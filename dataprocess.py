...
def make_edge_smooth(dataset_name, img_size) :
    check_folder('./dataset/{}/{}'.format(dataset_name, 'anime_smooth'))
    file_list = glob('./dataset/{}/{}/*.*'.format(dataset_name, 'anime_style'))
    save_dir = './dataset/{}/anime_smooth'.format(dataset_name)
...
def RGB2GRA(dataset_name, img_size):
    check_folder('./dataset/{}/{}'.format(dataset_name, 'anime_gray'))
    file_list = glob('./dataset/{}/{}/*.*'.format(dataset_name, 'anime_style'))
    save_dir = './dataset/{}/anime_gray'.format(dataset_name)

    for f in tqdm(file_list) :
        file_name = os.path.basename(f)

        gray_img = cv2.imread(f, 0)
        cv2.imwrite(os.path.join(save_dir, file_name), gray_img)
...
def main():
    ...
    RGB2GRA(args.dataset, args.img_size)
